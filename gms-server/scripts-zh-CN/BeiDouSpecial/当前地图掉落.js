...
function getMobImage(mob){
    ...
    if(type == null) {
        ...
        let w = mob.getStats().getImgwidth();
            h = mob.getStats().getImgheight();
            //为防止自定义的怪物图片没有添加尺寸值，任意值小于0则不展示图片，避免客户端假死。
        if ((w <= 0 || h <= 0) || (w > 160 && h > 250)) { //如果图片超过指定范围会造成客户端假死，因此这里需要替换成别的图片或者干脆不要。
